define('ace/mode/ink', [
  ...
  var inkHighlightRules = function() {
    ...
    this.$rules = {
      ...
      '#EXTERNAL': [
        {
          regex: /(\s*)(EXTERNAL\b)/,
          token: ['external', 'external.keyword'],
      ...
      '#inlineConditional': [
        {
          regex: /(\{)([^:\|\}]+:)/,
          token: ['logic.punctuation', 'logic.inline.conditional.condition'],
          push: [
            {
  ...
  (function() {
    ...
    this.getKnotFoldRange = function(session, foldStyle, row, line) {
      ...
      return this.getRangeFromStartToRegex(
        session,
        foldStyle,
        row,
        line,
    ...
    this.getRangeFromStartToRegex = function(
      session,
      foldStyle,
      row,
      line,
